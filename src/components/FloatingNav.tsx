...
import { Settings, Newspaper, Gamepad2 } from 'lucide-react';
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes fnav-glow-arena {
          0%,50%,100% { box-shadow: -2px 0 6px rgba(0,255,204,0.05); }
          25%          { box-shadow: -6px 0 32px rgba(0,255,204,0.70), 0 0 14px rgba(0,255,204,0.30); }
        }
        @keyframes fnav-spin {
          from { transform: rotate(0deg); }
          to   { transform: rotate(360deg); }
        }
        @keyframes fnav-controller-pulse {
          0%,80%,100% { transform: scale(1) rotate(0deg); }
          83%          { transform: scale(1.22) rotate(-10deg); }
          88%          { transform: scale(1.18) rotate(8deg); }
          93%          { transform: scale(1.10) rotate(-4deg); }
        }

        /* Confete para o botão de Arena */
        .fnav-cp {
          position: absolute;
        ...
        .fnav-cp1 { background:#00ffcc; animation: fnav-conf-1 6s ease-out 2.0s infinite; }
        .fnav-cp2 { background:#a78bfa; animation: fnav-conf-2 6s ease-out 2.12s infinite; }
        .fnav-cp3 { background:#f87171; animation: fnav-conf-3 6s ease-out 2.24s infinite; }
        .fnav-cp4 { background:#fbbf24; animation: fnav-conf-4 6s ease-out 2.36s infinite; }
        ...
        .fnav-maker {
          animation: fnav-knock 10s ease-in-out 1.5s infinite, fnav-glow-yellow 4s ease-in-out 0s infinite;
        }
        .fnav-news {
          animation: fnav-knock 10s ease-in-out 6.5s infinite, fnav-glow-blue 4s ease-in-out 2s infinite;
        }
        .fnav-arena {
          animation: fnav-knock 10s ease-in-out 4s infinite, fnav-glow-arena 4s ease-in-out 1s infinite;
        }
        .fnav-arena .fnav-ctrl {
          animation: fnav-controller-pulse 6s ease-in-out 2s infinite;
        }
      ...
      <div className="hidden md:flex flex-col items-end fixed right-0 top-1/2 -translate-y-1/2 z-50 gap-3">
        ...
        >
          <span className="fnav-label" style={{ color: isDark ? '#FFD600' : '#92400e' }}>Cultura Maker</span>
        ...
        >
          <span className="fnav-label" style={{ color: isDark ? '#60a5fa' : '#1e40af' }}>Notícias Tech</span>
          <div className="fnav-icon">
        ...
        <button
          ...
          aria-label="Arena de Desafios"
          className="fnav-btn fnav-arena focus:outline-none"
          style={{
            background: isDark
              ? 'linear-gradient(135deg, #041210 0%, #062420 100%)'
              : 'linear-gradient(135deg, #ffffff 0%, #f0fdf9 100%)',
            border: `1.5px solid ${isDark ? 'rgba(0,255,204,0.35)' : 'rgba(0,180,140,0.35)'}`,
          }}
        >
          <span className="fnav-label" style={{ color: isDark ? '#00ffcc' : '#0d9488' }}>Arena de Desafios</span>
          <div className="fnav-icon">
            <Gamepad2 className="fnav-ctrl w-5 h-5" style={{ color: isDark ? '#00ffcc' : '#0d9488' }} />
      ...
      <div className="md:hidden fixed bottom-[88px] right-4 z-50 flex flex-col gap-2.5">
        <button
          ...
          style={{
            background: isDark ? 'linear-gradient(135deg, #1C1408, #2d1f06)' : 'linear-gradient(135deg, #ffffff, #fef9ec)',
        ...
        <button
          ...
          className="w-11 h-11 rounded-full flex items-center justify-center active:scale-95 transition-transform"
          style={{
            background: isDark ? 'linear-gradient(135deg, #0a1628, #0f2040)' : 'linear-gradient(135deg, #ffffff, #eff6ff)',
        ...
        <button
          ...
          className="w-11 h-11 rounded-full flex items-center justify-center active:scale-95 transition-transform"
          style={{
            background: isDark ? 'linear-gradient(135deg, #041210, #062420)' : 'linear-gradient(135deg, #ffffff, #f0fdf9)',
            border: `1.5px solid ${isDark ? 'rgba(0,255,204,0.55)' : 'rgba(0,180,140,0.55)'}`,
            boxShadow: '0 4px 14px rgba(0,255,204,0.30)',
          }}
        >
          <Gamepad2 className="w-4 h-4" style={{ color: isDark ? '#00ffcc' : '#0d9488' }} />
        </button>
      </div>
    </>
  );
